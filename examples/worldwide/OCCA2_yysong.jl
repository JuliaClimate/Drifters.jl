...
using Drifters, Climatology, MITgcm, CSV, JLD2, NetCDF, Glob
...
"""
    ...
    filelist=basename.(glob("state_3d_set1*.data",D.pth))
    tmp1 = read_mdsio(joinpath(D.pth,filelist[m0]),:THETA)
    θ0=read(tmp1(:,:,D.k),P.u0.grid)
...
function update_FlowFields!(P::uvwMeshArrays,D::NamedTuple,t::AbstractFloat)
    ...
    println("The file index0 is ",m0)
    println("The file index1 is ",m1)
    ...
    list_w=basename.(glob("trsp_3d_set1*.data",D.pth))
    tmp1 = read_mdsio(joinpath(D.pth,list_w[m0]),:WVELMASS)
    w0=read(tmp1,P.u0.grid)
    w0=velocity_factor*w0
    ...
    tmp1 = read_mdsio(joinpath(D.pth,list_w[m1]),:WVELMASS)
    w1=read(tmp1,P.u0.grid)
    w1=velocity_factor*w1
    ...
    P.v1[:,:]=Float32.(v1[:,:])
    for k=1:nr
        tmpw=MeshArrays.exchange_main(-w0[:,k],1).MA
        P.w0[:,k]=Float32.(tmpw./D.Γ.DRC[k])
        tmpw=MeshArrays.exchange_main(-w1[:,k],1).MA
        P.w1[:,k]=Float32.(tmpw./D.Γ.DRC[k])
    end
    P.w0[:,1]=0*MeshArrays.exchange_main(-w0[:,1],1).MA
    P.w1[:,1]=0*MeshArrays.exchange_main(-w1[:,1],1).MA
    P.w0[:,nr+1]=0*MeshArrays.exchange_main(-w0[:,nr],1).MA
    P.w1[:,nr+1]=0*MeshArrays.exchange_main(-w1[:,nr],1).MA

    filelist=basename.(glob("state_3d_set1*.data",D.pth))
    tmp1 = read_mdsio(joinpath(D.pth,filelist[m0]),:THETA)
    θ0=read(tmp1,P.u0.grid)
    θ0[findall(isnan.(θ0))]=0.0 #mask with 0s rather than NaNs
    D.θ0[:,:]=Float32.(θ0[:,:])

    tmp1 = read_mdsio(joinpath(D.pth,filelist[m1]),:THETA)
    θ1=read(tmp1,P.u0.grid)
    θ1[findall(isnan.(θ1))]=0.0 #mask with 0s rather than NaNs
    D.θ1[:,:]=Float32.(θ1[:,:])

    tmp1 = read_mdsio(joinpath(D.pth,filelist[m0]),:SALT)
    S0=read(tmp1,P.u0.grid)
#    S0[findall(isnan.(S0))]=0.0 #mask with 0s rather than NaNs
    D.S0[:,:]=Float32.(S0[:,:])

    tmp1 = read_mdsio(joinpath(D.pth,filelist[m1]),:SALT)
    S1=read(tmp1,P.u0.grid)
#    S1[findall(isnan.(S1))]=0.0 #mask with 0s rather than NaNs
    D.S1[:,:]=Float32.(S1[:,:])
...
function read_velocities(γ::gcmgrid,t::Int,pth::String)
    filelist=basename.(glob("trsp_3d_set1*.data",pth))
...
"""
    init_FlowFields(;k=1,t)

Set up Global Ocean particle simulation in 2D with seasonally varying flow field.
"""
function init_FlowFields(; k=1,t, backward_time=false)

#  Climatology.get_ecco_velocity_if_needed()    # invalid website
#  add specific variables -- successful
#  Climatology.get_ecco_variable_if_needed("UVELMASS")
#  Climatology.get_ecco_variable_if_needed("VVELMASS")
#  Climatology.get_ecco_variable_if_needed("WVELMASS")
...
#  Climatology.get_ecco_variable_if_needed("SALT")
  ...
  pth = "/Users/yysong/Desktop/data/OCCA2/OCCA2HR1_diags"
  P,D=setup_FlowFields(k,Γ,func,pth,backward_time)
  D.🔄(P,D,t)
...
custom∫(prob) = Drifters.ensemble_solver(prob,solver=Tsit5(),reltol=1e-5,abstol=1e-5)
