...
# ╔═╡ 6384fa14-8144-11f1-8aeb-ed6131d15693
begin
	using LinearAlgebra 
...
# ╔═╡ b28189b2-2056-49da-92d5-8ddec1de6c87
include("stoch_tool.jl");

# ╔═╡ da6d3c82-4a85-47aa-a800-dde322291f7c
function make_coords(z)
    RF  = z                           # faces, length 51
    RC  = 0.5 .* (z[1:end-1] .+ z[2:end])   # cell centers, length 50
    DRF = abs.(diff(z))              # cell thickness, length 50
    DRC = abs.(diff(RC))        
	DRC = [abs(RC[1]); DRC]
    return (; RF, RC, DRC, DRF)      # NamedTuple
end

# ╔═╡ 9abc3829-3d3f-4d84-b171-929288dc2a91
function interannual_mixing_kappa(t, z; u_rms=5e-4, baseline=5e-15, lyear=365.0*86400,
                                   summer=30.0, maxdepth=1000.0, p1=1.0, p2=0.2)
    interannual = maxdepth .* sin.(fld.(t, lyear) .* 3) ./ 1.618
    doy = (t .% lyear) ./ lyear
    nd = doy.^p1 .* (1 .- doy).^p2
    nd_max = (p1/(p1+p2))^p1 * (p2/(p1+p2))^p2
    depth = -summer .- (maxdepth .- summer .+ interannual) .* nd ./ nd_max
    to_bottom = max.(z .- depth', 0.0)
    return 2.0 .* sqrt.(max.(-z, 0.0) .* to_bottom) .* u_rms .+ baseline
end

# ╔═╡ b1f13f5e-aa6f-4aff-953c-392e856554ed
begin
	day = 86400.0
	t = (1:2000) .* day
	seconds = time
	z = 0:-50:-2000
	coords = make_coords(z)
end

# ╔═╡ c6e6735c-e616-4aea-9f5c-885c591cccc7
KAPPA = interannual_mixing_kappa(t, z[1:end-1])

# ╔═╡ 3d38a99c-13fa-4838-b248-d887724f01fe
heatmap(t./day, reverse(z[1:end-1]), KAPPA[end:-1:1, :],
        color=:thermal, ylims=(-1500, 0),
        xlabel="Time (day)", ylabel="Depth (m)",
        colorbar_title="Diffusivity (m²/s)")

# ╔═╡ f7619806-0a95-4b06-b6b9-c22581ff5478
function simulate_abit(KAPPA,u_0, seconds)
	total_num_days = Int(seconds[end]/day_length)
	🔴 = zeros(length(u_0), total_num_days)
	
	current_tim = 0.0
	current_loc = copy(u_0)
	println(length(current_loc),size(🔴))
	current_ent = 1
	
	for itime in 1:length(seconds)-1
		updator = create_updator(KAPPA[:,itime],coords)
		while current_tim < seconds[itime+1]
			🔴[:,current_ent] = current_loc
			stride = minimum([day_length,seconds[itime+1] - current_tim])
			current_loc = updator(current_loc,stride)
			current_tim += stride
			current_ent +=1
		end
	end
...
# ╔═╡ 4d418f93-4bc9-4a25-9c98-297027c6172c
function particle_density(z, 🔴)
    z_faces = sort(z)           # make sure edges are ascending
    n_t = size(🔴, 2)
    n_zbins = length(z_faces) - 1
    
    density = zeros(n_zbins, n_t)
    for it in 1:n_t
        h = fit(Histogram, 🔴[:, it], z_faces)
        density[:, it] .= h.weights
    end
    
    return density, z_faces
end

# ╔═╡ 3a7b21d1-1b89-4ddc-966a-18d04b84d609
u_0 = rand(5000).*-30.0

# ╔═╡ 43daf2a4-223f-46a3-a391-91ec288bbe1a
begin
	u0 = rand(2000) .* -30.0
	loc = copy(u0)
	density = zeros(length(z)-1, length(t)-1)
	for it in 1:length(t)-1
	    updator = create_hybrid_updator(KAPPA[:, it], coords)
	    global loc = updator(loc, day)
	    density[:, it] = fit(Histogram, loc, sort(z)).weights
	end
end

# ╔═╡ 41b5741c-f507-4c2e-bf43-ce27b3e1afd6
heatmap(t[1:end-1]./day, sort(z)[1:end-1], density,
        xlabel="Time (day)", ylabel="Depth (m)",
        colorbar_title="Particle count", clims=(0, 100))
...
manifest_format = "2.0"
project_hash = "58f3d1f49c87a9ab4a7dcc29dd0266c7165eafc4"
...
git-tree-sha1 = "13c993d338396ad9077b5e9405198684561c6d84"
...
git-tree-sha1 = "158450603fec708026aab29ae4dcc9976e3461ab"
...
git-tree-sha1 = "9189c9fdd586d5b71e586911e5b33895b046068c"
...
git-tree-sha1 = "c8d5a4ad31e0efbedeb3f3333f5ac99a74d9d60b"
...
git-tree-sha1 = "f09d1b6375a6a8506a16aa7e8de971b4bac96322"
...
git-tree-sha1 = "03e4ee4384f6a80702025790685cffa2aae620c4"
...
git-tree-sha1 = "e4eaf8574eded4a0d0173ae126ef56bd034bc547"
...
git-tree-sha1 = "151d0e2aed3e576e464e9cd775f35f2eb4a9cf3e"
...
git-tree-sha1 = "2b7cd7d50ada72098f99734b47acf9b804f429d5"
...
git-tree-sha1 = "fd93bac733e8354716f8cea902c32c9022a172d2"
uuid = "aefaaa88-39f2-4e89-b162-d61b7e8cc81b"
version = "2.0.3"

[[deps.StochasticDiffEqLevyArea]]
deps = ["LinearAlgebra", "Random", "SpecialFunctions"]
git-tree-sha1 = "95d69adcf8e7da47f521a1e7b6e6eaf4e2f4950a"
uuid = "90dbc90e-856a-4131-af2c-e8c5aa0f35b5"
version = "2.0.3"

[[deps.StochasticDiffEqLowOrder]]
deps = ["DiffEqBase", "DiffEqNoiseProcess", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "9f05cd348e97e8132a1587d8c804bdcda0dc7f63"
...
git-tree-sha1 = "d995c6e5f70efea07e354e2a5fb8e0e590175ddb"
...
git-tree-sha1 = "9a4463f0a85c1e2d919bd8625e6ec84764f6b5ee"
...
git-tree-sha1 = "ff8d42c9e7844ed91c9363e760230f3ecd09db4d"
...
git-tree-sha1 = "1f13ef3f034422ba61d8b651d6368120622d6460"
